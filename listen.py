...
def returnHandlerIfExists(midi, actionConfig):
    if midi.isController():
        ...
        return actionConfig[controllerNumber] if (controllerNumber in actionConfig) else False;
    else: 
...
def handleMidiAction(midi, actionConfig, verbose):
    ...
    if (verbose):
        print("Received input from Controller: ", midi.getControllerNumber()) if midi.isController() else print("Received input from:", midi.getMidiNoteName(midi.getNoteNumber()));
        print("Has event handler? ", bool(eventHandler))

    # If there is no handler for this note, do nothing.
    if (bool(eventHandler) == False): return;
...
def startListening(options, actionConfig):
